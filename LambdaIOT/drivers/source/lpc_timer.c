/**********************************************************************
* $Id: lpc_timer.c,v 1.3 2013/12/30 05:14:17 cvsuser Exp $      lpc_timer.c         2011-06-02
*//**
* @file     lpc_timer.c
 ...
 */
/*********************************************************************//**
 * @brief       Convert a timer number to timer register pointer 
 * @param[in]   TIMx Pointer to LPC_TIM_TypeDef, should be:
 *              - LPC_TIM0: TIMER0 peripheral
 *              - LPC_TIM1: TIMER1 peripheral
 *              - LPC_TIM2: TIMER2 peripheral
 ...
 * @return      The timer number (0 to 3) or -1 if register pointer is bad
 **********************************************************************/
LPC_TIM_TypeDef* TIM_GetPointer (uint8_t tnum) 
{
...
/*********************************************************************//**
 * @brief       Convert a timer number to IRQ Num 
 * @param[in]   The timer number (0 to 3) 
 * @return      The timer irq number 
 **********************************************************************/
uint32_t TIM_GetIRQNum (uint8_t tnum) 
{
    uint32_t irqnum;

	if(tnum > 3) return (uint32_t)-1;
    if(tnum == 0)
    {
        irqnum = TIMER0_IRQn;
    }
...
//根据定时器号关闭或打开内部模块电源
void TIM_ConfigPPWR(uint8_t tnum, FunctionalState NewState)
{
	uint32_t PPType;
    if(tnum == 0)
    {
        PPType = CLKPWR_PCONP_PCTIM0;
    }
    else if (tnum == 1)
    {
        PPType = CLKPWR_PCONP_PCTIM1;
    }
    else if (tnum == 2)
    {
        PPType = CLKPWR_PCONP_PCTIM2;
    }
    else
    {
        PPType = CLKPWR_PCONP_PCTIM3;
    }
