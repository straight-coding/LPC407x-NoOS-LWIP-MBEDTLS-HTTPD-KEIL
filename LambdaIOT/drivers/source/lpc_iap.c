/**********************************************************************
* $Id: lpc_iap.c,v 1.6 2014/03/02 08:35:19 cvsuser Exp $      lpc_iap.c           2011-11-21
*//**
* @file     lpc_iap.c
...
#define IAP_Call    (IAP) IAP_LOCATION
*/
void (*IAP_Call)(uint32_t *cmd,uint32_t *result) = (void (*)(uint32_t *cmd,uint32_t *result))IAP_LOCATION;
...
#if (_IAP_LOG_EN_ > 0)
	LogPrint(0, "sectorPrepare: [%lu, %lu]\r\n", start_sec, end_sec);
#endif
...
IAP_STATUS_CODE CopyRAM2Flash(uint8_t * dest, uint8_t* source, IAP_WRITE_SIZE size)
{
	...
	CPU_SR_ALLOC();
#if (_IAP_LOG_EN_ > 0)
	LogPrint(0, "ramToFlash: 0x%05X <== 0x%05X, size=%d(%05X)\r\n", dest, source, size, size);
#endif
