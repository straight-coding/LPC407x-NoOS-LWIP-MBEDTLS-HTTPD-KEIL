/**********************************************************************
* $Id: lpc_emac.c,v 1.7 2014/03/22 02:29:10 cvsuser Exp $      lpc_emac.c          2011-06-02
*//**
* @file     lpc_emac.c
 ...
 * @return        None
 **********************************************************************/
void EMAC_Write_PHY (uint16_t phyadd, uint8_t PhyReg, uint16_t Value)
{
    unsigned int tout;

    LPC_EMAC->MADR = ((phyadd & 0x1F) << 8 )| (PhyReg & 0x1F);
    LPC_EMAC->MWTD = Value;
 ...
 **********************************************************************/
uint16_t EMAC_Read_PHY (uint16_t phyadd, uint8_t PhyReg)
{
    unsigned int tout;
 ...
 * @return      SUCESS/ERROR/-1£¬ÆäÖÐ-1ÎªÐ¾Æ¬´íÎó
 *
 ...
 *  (Ref. from LPC17xx UM)
 **********************************************************************/
int32_t EMAC_Init(EMAC_CFG_Type *EMAC_ConfigStruct, uint32_t bIntEn)
{
    ...
    {
		tmp = EMAC_ConfigStruct->pfnPHYInit(&EMAC_ConfigStruct->PhyCfg);
//		if(tmp != SUCCESS)
//			return tmp;
    }
...
//    LPC_EMAC->RxFilterCtrl = EMAC_RFC_MCAST_EN | EMAC_RFC_BCAST_EN | EMAC_RFC_PERFECT_EN;
    LPC_EMAC->RxFilterCtrl = EMAC_RFC_UCAST_EN | EMAC_RFC_BCAST_EN | EMAC_RFC_PERFECT_EN;
...
void EMAC_SendPBuf(struct pbuf *p)
{
	int count = 0;
	unsigned short wordBuf;
	char* pFrom = (char*)&wordBuf;

	struct pbuf* q;
	unsigned short* pCopyTo;
	
	int size = p->tot_len;

	int idx  = LPC_EMAC->TxProduceIndex;
	pCopyTo = (unsigned short *)TX_DESC_PACKET(idx);
	TX_DESC_CTRL(idx) = size | EMAC_TCTRL_LAST;
...
struct pbuf* EMAC_ReceivePBuf(void)
{
	int len;
	struct pbuf* p = NULL;
	...
	{
		// Get data size
		long rxLen = EMAC_GetRxFrameSize();
		if (rxLen > 0)
		{
			// trip out 4-bytes CRC field, note that length in (-1) style format
			rxLen -= 3;
...
#endif
				p = pbuf_alloc(PBUF_RAW, len, PBUF_RAM); //PBUF_RAW: no header, offset=0
				
				pSource = (unsigned short*)EMAC_GetRxBuffer();
...
#endif
					ptrw = q->payload; //q->payload MUST be 2-byte aligned;
					left = q->len;
					while((rxLen > 0) && (left > 0))
					{
						*ptrw++ = *pSource++; //copy two bytes
						rxLen -= 2;
						left  -= 2;
