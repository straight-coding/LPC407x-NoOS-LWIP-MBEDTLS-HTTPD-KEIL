/**********************************************************************
* $Id: system_LPC407x_8x_177x_8x.c,v 1.2 2014/03/29 02:40:53 cvsuser Exp $		system_LPC407x_8x_177x_8x.c			2012-01-16
*//**
...
void RebootFromNewVTOR(unsigned long lNewVTOR)
{
	void (*fp)(void);
	unsigned long* pResetHandler = (unsigned long*)(lNewVTOR+4);

	NVIC->ICER[0] = 0xFFFFFFFF; /* disable interrupt */
	NVIC->ICER[1] = 0x00000007; /* disable interrupt */
	SYSTICK_Cmd(DISABLE);

	
	fp = (void (*)(void))(*pResetHandler);
	fp();
}

void RebootFromNewEntry(unsigned long lNewEntry)
{
	void (*fp)(void);
...
{  
	...
	unsigned int reg_BFAR = SCB->BFAR;//(*((volatile unsigned long *)(0xE000ED38)));  
	unsigned int reg_CFSR = SCB->CFSR;//(*((volatile unsigned long *)(0xE000ED28)));  
	unsigned int reg_HFSR = SCB->HFSR;//(*((volatile unsigned long *)(0xE000ED2C)));  
	unsigned int reg_DFSR = SCB->DFSR;//(*((volatile unsigned long *)(0xE000ED30)));  
	unsigned int reg_AFSR = SCB->AFSR;//(*((volatile unsigned long *)(0xE000ED3C)));  
	unsigned int reg_MMFAR = SCB->MMFAR;
	
	LogPrint(0, "[Hard fault handler]\n");
