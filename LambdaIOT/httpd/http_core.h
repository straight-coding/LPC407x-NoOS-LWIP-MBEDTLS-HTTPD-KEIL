...
#define MAX_REQ_BUF_SIZE				128		//max. length of the header that can be recognized, and it must be bigger than the max.length of path.
												//the larger the receiving buffer, the faster the uploading speed.
#define MAX_SEND_BUF_SIZE				256		//the larger sending buffer, the faster the downloading speed, and it must be bigger than the max.length of header.
