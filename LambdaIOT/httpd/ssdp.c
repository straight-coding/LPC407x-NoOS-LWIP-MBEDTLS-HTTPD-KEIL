...
static void ssdp_advertise(SSDP_TYPES arg);
...
static long long g_nLastAdvertise = 0;
static long g_nAdvertisePeriod = 0;
...
#if !NO_SYS
	tcpip_timeout(60 * 1000UL, (void (*)(void*))ssdp_advertise, (void*)ADVERTISE_UP);
#endif
	LWIP_DEBUGF(REST_DEBUG, ("ssdp: started"));
	
	g_nLastAdvertise = LWIP_GetTickCount();
	g_nAdvertisePeriod = 500;
}

void ssdpDown(void)
{
#if !NO_SYS
	tcpip_untimeout((void (*)(void*))ssdp_advertise, (void*)ADVERTISE_UP);
#endif
	sys_mutex_lock(&lock_ssdp);
	{
		ip_addr_t castIp;
...
void ssdp_advertise(SSDP_TYPES arg)
{
	ip_addr_t addr;
	addr.addr = 0;
	
	sys_mutex_lock(&lock_ssdp);
		ssdp_send_to(arg, &addr, 0);
	sys_mutex_unlock(&lock_ssdp);
...
{
	if (g_nAdvertisePeriod > 0)
	{
		if ((long)(tick - g_nLastAdvertise) > g_nAdvertisePeriod)
		{
			g_nLastAdvertise = tick;
			g_nAdvertisePeriod = 60 * 1000UL;
			
			ssdp_advertise(ADVERTISE_UP);
		}
	}
}
